...
캐시 사본이 최신인지 점검하는 신선도 검사를 재검사라 하고, 데이터를 최신 상태로 유지해준다. 캐시는 언제든지 사본을 재검사할 수 있으나 비효율적이므로 충분히 오래된 경우 재검사를 실시한다.  재검사가 이루어질 떄 `적중`과 `부적중`, `객체 삭제`가 이루어진다.
...
- **only-if-cached:** 오래된 경우에도 HTTP 캐시의 응답을 사용. HTTP 캐시에 응답이 없으면 오류가 발생. `mode: "same-origin"`과 함께 사용해야 한다.

> 📌 `must-revalidate`, `public`, `max-age=<seconds>` 등은 fetch의 `cache` 옵션이 아닌 `Cache-Control` 응답 헤더의 디렉티브다. 콤마로 구분해 `no-cache, must-revalidate, no-store` 처럼 섞어서 사용할 수 있다. 솔직히 복잡해서 섞어서 사용하겠나..
...
여담으로 만약 서버 비용을 본인이 낸다고 한다면 캐시 컨트롤이 아주 잘 될 것 같다. Tanstack Query 같은 라이브러리가 존재해서 다행이다.